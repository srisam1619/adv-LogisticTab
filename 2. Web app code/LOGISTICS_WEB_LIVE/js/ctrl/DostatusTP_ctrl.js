App.controller('DostatusTP_ctrl', ['$scope', '$rootScope', '$http', '$window', '$cookies','util_SERVICE','$location',
...
function ($scope, $rootScope, $http, $window, $cookies,US,$location) {
	...
	$scope.valdata = function()
	{
		...
		var itemqty = true;
		 ...
		 if(!itemqty)
		 {
			 alert("Do Qty should not Zero or empty.");
			 rvalue = false;
		 }

		 //var Collqty = true;
		 //for(var b=0;b<$scope.Items.length;b++)
	 	 //{
		 //	if($scope.Items[b].CollectedQty=="" || $scope.Items[b].CollectedQty=="0")
		 //	{
		 //		Collqty = false;
		 //	}
		 //} 
		 //if(!Collqty)
		 //{
		 //	 alert("Collected Qty should not Zero or empty.");
	  	 //	 rvalue = false;
		 //}

		  var DOstatusVal = true;
		  var TonerPercentageVal = true;
		 for(var m=0;m<$scope.Items.length;m++)
	 	 {
			...
			if($scope.Items[m].CollectedQty=="1")
			{
				if($scope.Items[m].TonerPercentage=="" || $scope.Items[m].TonerPercentage===undefined)
				{
					TonerPercentageVal = false;
		 ...
		 if(!TonerPercentageVal)
		 {
			 alert("Toner Percentage should not empty if Collected Qty 1");
			 rvalue = false;
		 }
	...
	$scope.setCollQty = function(id)
	{
		
		if($scope.Items[id].DeliveryStatus.Id=="Loaded")
		{
			$scope.Items[id].CollectedQty = "1";
		}
		else
		{

		 	$scope.Items[id].CollectedQty = $scope.Items[id].DOQty;
		}
	}
	...
	$scope.SaveDoData = function()
	{
		...
		if($scope.valdata())
		{
			$scope.MYAdditionalItems =angular.copy($scope.AdditionalItems);
			for(var k=0;k<$scope.Items.length;k++)
	 	 {
			 $scope.Items[k].DeliveryStatus = $scope.Items[k].DeliveryStatus.Id;
			 if($scope.Items[k].TonerPercentage=="")
			 	$scope.Items[k].TonerPercentage = "0";
			 var additem = {"DocEntry":$scope.Items[k].DocEntry,"DocNum":$scope.Items[k].DocNum,"ItemCode":$scope.Items[k].ItemCode,"ItemDescription":$scope.Items[k].ItemDescription,"Qty":$scope.Items[k].CollectedQty,"ItemCondition":"Used"};
			 $scope.MYAdditionalItems.push(additem);
		 }
		
		US.UpdateDeliveryOrderTP($scope.Header,$scope.AttachmentData,$scope.Items,$scope.MYAdditionalItems,$scope.user[0].CompanyCode)
 ...
 $scope.applytoall = function(status)
 {
	 ...
	 for(var j=0;j<$scope.Items.length;j++)
	 {
		 ...
		 if(status.Id=="Loaded")
			$scope.Items[j].CollectedQty = "1";
		 else
		 	$scope.Items[j].CollectedQty = angular.copy($scope.Items[j].DOQty);
	 }
